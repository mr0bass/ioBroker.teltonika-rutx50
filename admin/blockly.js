...
Blockly.Words['sms_help'] = {
    ...
    "zh-cn": "https://github.com/iobroker-community-adapters/ioBroker.teltonika-rutx50/blob/master/README.md"
};
    ...
    init: function() {
        ...
        if (typeof main !== 'undefined' && main.instances) {
            for (let i = 0; i < main.instances.length; i++) {
                const m = main.instances[i].match(/^system.adapter.teltonika-rutx50.(\d+)$/);
                if (m) {
        ...
        this.appendValueInput('TO')
            .appendField(Blockly.Translate('sms_to'));
...
Blockly.JavaScript['sms'] = function(block) {
    ...
    const recipient = Blockly.JavaScript.valueToCode(block, 'TO', Blockly.JavaScript.ORDER_ATOMIC);
    ...
    const logText = logLevel ? ', (error, result) => {\n' +
        '  if (error) {\n' +
        '    console.' + logLevel + '("SMS error: " + error);\n' +
        '  } else {\n' +
